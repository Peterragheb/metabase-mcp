...
export class MetabaseServer {
  ...
  /**
   * Connect this MCP server to a Streamable HTTP transport (one MetabaseServer instance per session).
   */
  async connectHttpTransport(transport: StreamableHTTPServerTransport): Promise<void> {
    await this.server.connect(transport);
  }
}
